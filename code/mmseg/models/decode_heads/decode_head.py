...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def local_avg_tp_fn_fp(self, f, mask, fn, fp, logit):
        ...
        with torch.no_grad():
            ...
            index = index.float()
            #mask = mask*logit
            mask = mask/(torch.norm(mask,p=1,dim=1,keepdim=True) + 1e-12)
    ...
    def get_score(self, feature, lbl_one, logit, avg_f_tp, f_fn, f_fp):
        ...
        avg_f_tp = avg_f_tp/(torch.norm(avg_f_tp, p=2, dim=2, keepdim=True)+1e-12)
    ...
    def cl_forward(self, feature, gt, logit):
        ...
        avg_f_tp, f_fn, f_fp = self.local_avg_tp_fn_fp(feature, mask, fn, fp, logit_detach)
