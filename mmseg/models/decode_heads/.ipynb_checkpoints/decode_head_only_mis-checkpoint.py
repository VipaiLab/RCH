...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    def __init__(self,
        ...
        if use_cl:
            cl_type = 'CrossEntropyLoss'
            self.loss_cl = build_loss(dict(type=cl_type,use_sigmoid=False,loss_weight=0.1))   #8192,min_kept=10000
    ...
    def get_mis(self, lbl, pred):
        mis = lbl.clone()
        mis[pred == lbl] = self.ignore_index
        self.mis = mis
    ...
    def cl_forward(self, feature, lbl, logit):
        ...
        return {"cl_fn": score_cl_fn, "cl_fp": score_cl_fp, "gt": self.mis}
    ...
    def losses(self, seg_logit, seg_label, cl_logits):
        ...
        if self.sampler is not None:
            ...
            if self.use_cl:
                cl_weight = None
                cl_fn = cl_logits["cl_fn"]
                cl_fp = cl_logits["cl_fp"]
                gt = cl_logits["gt"].long()
                #cl_fn = resize(
        ...
        if self.use_cl:
            loss['loss_fn'] = self.loss_cl(
                ...
                weight=cl_weight,
                ignore_index=self.ignore_index)
            loss['acc_fn'] = accuracy(cl_fn, gt)
            loss['loss_fp'] = self.loss_cl(
                cl_fp,
                gt,
                weight=cl_weight,
                ignore_index=self.ignore_index)
            loss['acc_fp'] = accuracy(cl_fp, gt)
        return loss
