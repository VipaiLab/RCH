...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    def __init__(self,
        ...
        #if use_cl:
            #self.loss_cl = build_loss(dict(type=cl_type,use_sigmoid=False,loss_weight=0.1))
        self.aux_weight = 0.1
    ...
    def local_avg_tp_fn_fp(self, f, mask, fn, fp, logit):
        ...
        d = f.size(1)
        #f = f.reshape(b,d,h*w)
        f = f.permute(1,0,2,3).reshape(d,b*h*w)
        avg_f = self.avg_f.detach()

        #fn = fn.reshape(b,h*w,c)
        #fn = fn/(torch.norm(fn,p=1,dim=1,keepdim=True) + 1e-12)
        #f_fn = torch.bmm(f,fn)
        #fp = fp.reshape(b,h*w,c)
        #fp = fp/(torch.norm(fp,p=1,dim=1,keepdim=True) + 1e-12)
        #f_fp = torch.bmm(f,fp)
        fn = fn.reshape(b*h*w,c)
        fn = func.normalize(fn,p=1,dim=0)
        f_fn = torch.matmul(f,fn).repeat(b,1,1)
        
        fp = fp.reshape(b*h*w,c)
        fp = func.normalize(fp,p=1,dim=0)
        f_fp = torch.matmul(f,fp).repeat(b,1,1)


        #f = f.permute(1,0,2).view(d,b*h*w)
    ...
    def losses(self, seg_logit, seg_label, cl_logits):
        ...
        if not self.use_cl:
            ...
            loss['loss_seg'], loss_fn, loss_fp = self.loss_decode(
                [seg_logit, cl_fn, cl_fp],
                seg_label,
                weight=cl_weight,
                ignore_index=self.ignore_index)
            loss['acc_seg'] = accuracy(seg_logit, seg_label)

            loss['loss_fn'], loss['loss_fp'] = self.aux_weight * loss_fn, self.aux_weight * loss_fp
