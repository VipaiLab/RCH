...
class OCRHead(BaseCascadeDecodeHead):
    ...
    def __init__(self, ocr_channels, scale=1, **kwargs):
        ...
        if self.use_cl:
            ...
            self.begin_channel = sum(self.in_channel_lis[0:1])
            self.end_channel = self.begin_channel + num_channel
            if self.num_classes > num_channel:
                embed_channel = num_channel*2
            else:
                embed_channel = num_channel
            print('num embed_channels is ', embed_channel)
            avg_f = torch.randn(embed_channel, self.num_classes)
